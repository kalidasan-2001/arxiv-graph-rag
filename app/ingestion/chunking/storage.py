"""Local filesystem storage for structured chunk artifacts (`chunks.json`).

Lives alongside `paper.pdf` and `parsed.json` at the same deterministic
paper-version directory (Prompt 4/5)::

    {PAPER_STORAGE_PATH}/{source}/{source_id}/{version}/chunks.json

Same atomicity discipline: writes go to a `.part` temp file first, and
only an `os.replace` (`Path.replace`) moves a fully-serialized, complete
file into the final name -- a partial crash can never leave a `chunks.json`
that looks complete (prompt #22).
"""
...
class ChunkArtifactStorage:
    """Filesystem-backed storage for structured `chunks.json` artifacts."""

    _FINAL_NAME = "chunks.json"
    _TEMP_SUFFIX = ".part"
    ...
    def write(
        self, document: ChunkedPaperDocument, *, source: str, source_id: str, version: str
    ) -> Path:
    ...
    def try_read(
    ...
    ) -> ChunkedPaperDocument | None:
        """Read and validate `chunks.json`.

        Returns `None` if missing *or* corrupt (malformed JSON, schema
        mismatch) rather than raising -- callers treat both cases
        identically: "no valid chunk artifact exists here" (prompt #30).
        """
        ...
        try:
            ...
            return ChunkedPaperDocument.model_validate(payload)
